...
def runSims(sims = 10,steps = 10,verbosity = 2,simIdent = 'Test'):
	...
	for count in range(0,sims):
		...
		for step in range(0,steps):
			...
			if(verbosity>=3):
				if(act > 3):
					actMap = {4:'Is the Target East of Me?',5:'Is the Target West of Me?',6:'Is the Target North of Me?',7:'Is the Target South of Me?',8:'Is the Target on the Road?',9:'Is the Target off-road?'}
		...
		print(""); 
		np.save('../../data/dataGridSQuest_E2_{}'.format(simIdent),dataPackage)
