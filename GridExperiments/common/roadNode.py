...
def populatePoints(g,N = 100):
	...
	for i in range(0,N):
		...
		tmp = [0,0]; 
		tmp[0] = (goals[i].loc[0]-curs[i].loc[0])*np.random.random() + curs[i].loc[0]; 
		tmp[1] = (goals[i].loc[1]-curs[i].loc[1])*np.random.random() + curs[i].loc[1]; 
		s[i] = tmp; 
...
def simPoints(netFile,N = 100, T = 100,hists = True,populate = True):
	...
	for i in range(0,T):
		...
		sp = np.array(s).T; 
		if(hists):
			ax2.hist2d(sp[0],sp[1],bins=50,range=[[-.2,8.2],[-.2,8.2]],cmin=1,cmap='Reds',zorder=2);
...
def displayNetworkMap(netFile,fig = None,ax=None, vis = True, redraw = False):
	...
	cs = {"HighRoad":"#FFF2AF","HighEdge":"#F6CF65","Road":"#FFFFFF","RoadEdge":"#D5D8DB","Empty":"#E8E8E8","Forest":"#C3ECB2","Water":"#AADAFF"}
	roadSize = 3; 
	riverSize = 4
	...
	if('Extent' in fil.keys()):
		e = fil['Extent']; 
		ax.add_patch(Rectangle((-.2,-.2),e[0]+.2,e[1]+.2,fill=True,color=cs['Empty']))
	else:
		ax.add_patch(Rectangle((-.2,-.2),8.4,8.4,fill=True,color=cs['Empty']))
	...
	if('Forests' in fil.keys()):
		...
		for f in forests.keys():
			ax.add_patch(Rectangle(forests[f]['lowLeft'],forests[f]['width'],forests[f]['height'],fill=True,color=cs['Forest']));
	...
	if("Water" in fil.keys()):
		...
		for r in rivers.keys():
			ax.plot([rivers[r]['start'][0],rivers[r]['end'][0]],[rivers[r]['start'][1],rivers[r]['end'][1]],linewidth=riverSize,color=cs['Water']); 

		for l in lakes.keys():
			ax.add_patch(Circle(lakes[l]['loc'],radius=lakes[l]['rad'],fill=True,color=cs['Water']))
	...
	for node in net:
		...
		for nei in node.neighbors:
			if([node.ident,nei.ident] in allHighNodes):
				#plot as high road
				ax.plot([node.loc[0],nei.loc[0]],[node.loc[1],nei.loc[1]],linewidth=roadSize+2,color=cs['HighEdge']); 
				ax.plot([node.loc[0],nei.loc[0]],[node.loc[1],nei.loc[1]],linewidth=roadSize,color=cs['HighRoad']);
			else:
				ax.plot([node.loc[0],nei.loc[0]],[node.loc[1],nei.loc[1]],linewidth=roadSize+2,color=cs['RoadEdge']); 
				ax.plot([node.loc[0],nei.loc[0]],[node.loc[1],nei.loc[1]],linewidth=roadSize,color=cs['Road']);
