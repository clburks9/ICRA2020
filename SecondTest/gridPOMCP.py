...
class POMCP:
	...
	def resampleNode(self,h):
		...
		while(len(b) < maxTreeQueries):
			...
			tmp[2] += (tmp[5].loc[0]-tmp[4].loc[0])*np.random.random()*.025 # + np.random.normal(0,dev); 
			tmp[3] += (tmp[5].loc[1]-tmp[4].loc[1])*np.random.random()*.025
...
def simForward(steps = 10):
	...
	#make belief
	network = readInNetwork('flyovertonNetwork.yaml')
	target,curs,goals = populatePoints(network,maxTreeQueries); 
	pickInd = np.random.randint(0,len(target)); 
	trueS = [np.random.random()*8,np.random.random()*8,target[pickInd][0],target[pickInd][1],curs[pickInd],goals[pickInd]]; 

	sSet = []; 
	for i in range(0,len(target)):
		sSet.append([trueS[0],trueS[1],target[i][0],target[i][1],curs[i],goals[i]]); 
	...
	for step in range(0,steps):
		fig,ax1 = displayNetworkMap('flyovertonNetwork.yaml',fig,ax1,False,redraw=True);

		allPrevs[step] = np.array(trueS); 
		act = solver.search(sSet,h,False,off=False);
		...
		ax2.hist2d(sp[0],sp[1],bins=40,range=[[-.2,8.2],[-.2,8.2]],cmin=1,cmap='inferno',zorder=2);
		...
		ax2.scatter(allPrevs[step][2],allPrevs[step][3],c=[0,1,0],zorder = 3); 
		...
		ax2.arrow(allPrevs[step][2],allPrevs[step][3],trueS[2]-allPrevs[step][2],trueS[3]-allPrevs[step][3],edgecolor=[0,1,0],head_width = 0.25,facecolor = [0,.5,0],zorder=3); 
	...
	fig.suptitle("Estimates with 2 sigma bounds when caught at step: {}".format(step)); 
...
def runSims(sims = 10,steps = 10,verbosity = 1,simIdent = 'Test'):
	...
	dataPackage = {'Meta':{'NumActs':numActs,'maxDepth':maxDepth,'c':c,'maxTreeQueries':maxTreeQueries,'maxTime':maxTime,'gamma':gamma,'numObs':numObs,'problemName':problemName,'fileName':'unifiedPOMCP'},'Data':[]}
